...
<workflow>
    <step name="work dir setup">
        use ~/.local/share/btca-sandbox as the place where you clone/search repos
    </step>
    <step name="load">
        if the repo(s) are already in the work dir ~/.local/share/btca-sandbox update them, otherwise clone them. clone the main branch by default, unless the user asks for something else
    </step>
    <step name="search">
...
Your job is to search the working directory ~/.local/share/btca-sandbox at the top level, just to get a list of all the repos that have been previously cloned
