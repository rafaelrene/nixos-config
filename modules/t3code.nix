...
let
  ...
  updaterSource = pkgs.runCommand "t3code-updater-source" { } ''
    mkdir -p "$out"
    cp ${../packages/t3code/flake.nix} "$out/flake.nix"
    cp ${../packages/t3code/package.nix} "$out/package.nix"
  '';
...
in
{
  ...
  systemd.tmpfiles.rules = [
    ...
    "C ${baseDir}/userdata/settings.json 0600 raf raf - ${../config/t3code-settings.json}"
  ];
}
