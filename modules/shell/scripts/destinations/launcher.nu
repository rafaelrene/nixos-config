...
def activate [settings: record, destination: record, force_new: bool] {
    ...
    if $settings.platform == darwin {
        let result = (
            ^/usr/bin/osascript $settings.appleScript new $destination.kind $destination.path $settings.ssh
            | complete
        )
        if $result.exit_code != 0 { error make {msg: $result.stderr} }
    } else {
        let args = if $destination.kind == ssh {
            [
                $"--working-directory=($settings.home)"
                $"--title=SSH: ($destination.path)"
                -e
                $settings.ssh
                $destination.path
            ]
        ...
        let result = (
            ^niri msg action spawn -- $settings.ghostty --gtk-single-instance=true ...$args
            | complete
        )
        if $result.exit_code != 0 { error make {msg: $result.stderr} }
    }
...
def pick [settings: record] {
    ...
    let result = (
        $labels
        | ^vicinae dmenu --placeholder 'Open destination…' --no-quick-look
        | complete
    )
